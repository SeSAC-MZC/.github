## (영등포8기) AWS와 AI를 활용한 MSA 기반 웹 서비스 개발

### 환영합니다! 여러분 모두 개발자가 되는 그 날까지 화이팅! 🏃🏃‍♂️🏃‍♀️

### 👨‍🏫 훈련교사 소개
  - 이름(성별) : 민경태(남)
  - 연락처 : 010-8725-4966 / gt_min@naver.com
  - 주요 자격 : 정보처리기사, SQLD, 직훈교사 정보기술개발/인공지능 등

### 📖 교과목 / 훈련시간
  1. 웹서비스 개발을 위한 프로그래밍 기본 다지기 / 60H
  2. Spring Boot 프레임워크와 Spring JPA & MyBatis / 60H
  3. AI를 활용한 Front-End 개발 기술 / 60H
  4. 미니 프로젝트 / 90H
  5. 클라우드의 시작, Linux & Network / 24H
  6. 클라우드 핵심 인프라와 네트워크 및 서비스 설계와 구축 / 18H
  7. MSA 기반 Microservice 관리를 위한 Docker & Kubernetes 활용 / 60H
  8. MSA 기반 Spring Cloud / 60H
  9. AWS EKS 운영 / 30H
  10. AWS 클라우드 네이티브 Serverless 개발 / 60H
  11. AWS AI 서비스 활용 / 18H
  12. 프로젝트 / 90H
  13. 최종 프로젝트 / 87H
  14. 창업교육 / 3H

### 📂 수업 소스코드
  - [JavaStudy](https://github.com/SeSAC-MZC/JavaStudy)
  - [DbStudy](https://github.com/SeSAC-MZC/DbStudy)
  - [SpringBootStudy](https://github.com/SeSAC-MZC/SpringBootStudy)
  - [JavaScriptStudy](https://github.com/SeSAC-MZC/JavaScriptStudy)
  - [ReactStudy](https://github.com/SeSAC-MZC/ReactStudy)

### 📂 노션
  - [SeSAC-MZC](https://www.notion.so/SeSAC-MZC-364dc48d864180ca85f8fdda76c8e24b)

### 🖇️ HTML/CSS 학습 링크
  - [HTML/CSS 학습사이트](https://html-css-app-jet.vercel.app/)

### 📂 웨일온 전용회의실
  - [링크(네이버 웨일 브라우저 설치 필요)](https://whaleon.us/o/CSrzPM/04d0d57561814ee5bae5d0d882ce7e03)
  - 회의 ID: 210 868 4802
  - 비밀번호: 162305

### 🆘 매니저님과의 오픈채팅
  - 질문 전용 오픈채팅
  - https://open.kakao.com/o/sreGhNyi

### 🧑‍🤝‍🧑 1차프로젝트 주제
- **추천 API (*아래 예시를 꼭 지키기 않아도 무방합니다.*)**
  1. [한국관광공사_국문 관광정보 서비스_GW](https://www.data.go.kr/data/15101578/openapi.do)
  2. [한국관광공사_고캠핑 정보 조회서비스_GW](https://www.data.go.kr/data/15101933/openapi.do)
  3. [소상공인시장진흥공단_상가(상권)정보_API](https://www.data.go.kr/data/15012005/openapi.do)
  4. [한국고용정보원_온통청년_청년정책API](https://www.data.go.kr/data/15143273/openapi.do)
  5. [농림축산식품부 농림축산검역본부_국가동물보호정보시스템 구조동물 조회 서비스](https://www.data.go.kr/data/15098931/openapi.do)
  6. [예술경영지원센터 공연예술통합전산망:KOPIS](https://kopis.or.kr/por/cs/openapi/openApiInfo.do?menuId=MNU_00074)
  7. [국립중앙도서관 도서관 정보나루 API](https://www.data4library.kr/apiUtilization)
  8. [카카오 책 검색 API](https://developers.kakao.com/docs/ko/daum-search/common)

- **SNS 로그인 (*아래 API는 모두 사용합니다.*)**
  1. [카카오 로그인](https://developers.kakao.com/docs/ko/kakaologin/common)
  2. [네이버 로그인](https://developers.naver.com/docs/login/overview/overview.md)
  3. [구글 로그인](https://developers.google.com/identity/protocols/oauth2?hl=ko)

- **주요 고려사항**
  1. API 활용 신청 (개발용)
  2. 메인 화면(Using API) / 검색 (무한스크롤)
  3. SNS 로그인 / 회원가입
  4. 게시판 / 댓글 / 좋아요 (유형: 댓글형 or 계층형 or 첨부형, 삭제 전략, 페이징)
  5. 관리자 기능 (별도 화면, 모든 걸 할 수 있는 관리자)
  6. 주제별 핵심 비즈니스 (예약, 데이터 파싱, 검색 등)
  7. API 네트워크 오류 대비 (캐싱)

- **조별 주제**
  - 1조: 관광 API
  - 2조: 도서 API
  - 3조: 유기동물 API
  - 4조: 청년정책 API
